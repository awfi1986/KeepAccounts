	...
	var DtPicker = $.DtPicker = $.Class.extend({
		...
		_createYear: function(current) {
			...
			if (options.customData.y) {
				...
				var dDate = new Date();
				var yBegin = dDate.getYear();
				if (yBegin < 1000) yBegin += 1900;
				var yEnd = yBegin+1;
		...
		_createDay: function(current) {
			...
			if (options.customData.d) {
				...
				//console.log(maxDay);
				var dDate = new Date();
				var st = dDate.getDate();
				
				var end = st+7;
				for (var d = st; d <= maxDay; d++) {
			...
			//console.log(ui.d.picker.getSelectedValue());
